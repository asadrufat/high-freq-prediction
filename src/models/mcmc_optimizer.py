...
class MCMCOptimizer:
    ...
    def _prepare_data(
        ...
        scaled_index = (clean_index - scaling_params['index_mean']) / scaling_params['index_std']
        scaled_multipliers = (clean_multipliers - scaling_params['mult_mean']) / scaling_params['mult_std']
    ...
    def fit_predict(
        ...
        """
        # Prepare data
        scaled_multipliers, scaled_index, scaling_params = self._prepare_data(multipliers, index)
        ...
        with pm.Model() as model:
            ...
            ar1_like = pm.AR1('ar1_like',
                            phi=phi,
                            sigma=sigma,
                            mu=mu,
                            observed=scaled_multipliers)
        ...
        scaled_future_x = (future_x - scaling_params['index_mean']) / scaling_params['index_std']
        ...
        predictions = predictions * scaling_params['mult_std'] + scaling_params['mult_mean']
        
        credible_intervals = {
            'lower': np.percentile(posterior_pred['ar1_like'], 2.5, axis=0) * scaling_params['mult_std'] + scaling_params['mult_mean'],
            'upper': np.percentile(posterior_pred['ar1_like'], 97.5, axis=0) * scaling_params['mult_std'] + scaling_params['mult_mean']
        }
