...
def save_results(
    metrics: Dict[str, float], multipliers: np.ndarray, date: str, output_path: str
) -> None:
    """
...
def evaluate_predictions(
    ...
    for hour in sorted(set(hours)):
        mask = hours == hour
        if mask.sum() > 0:
            results["by_hour"][hour] = calculate_metrics(y_true[mask], y_pred[mask])
    ...
    for day in sorted(set(days)):
        ...
        if mask.sum() > 0:
            results["by_day"][str(day)] = calculate_metrics(y_true[mask], y_pred[mask])
...
def generate_prediction_report(
    evaluation_results: Dict, output_path: str, date: str
) -> None:
    """
...
def calculate_business_impact_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, cost_per_miss: float = 1.0
) -> Dict[str, float]:
    """
    ...
    under_prediction_cost = (
        np.sum(under_prediction) * cost_per_miss * 1.5
    )  # Higher penalty for under-prediction
    ...
    return {
        ...
        "average_cost_per_period": float(
            (over_prediction_cost + under_prediction_cost) / len(y_true)
        ),
    }
