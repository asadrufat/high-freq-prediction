...
class FeatureGenerator:
    ...
    def _create_cyclical_features(self, df: pd.DataFrame, feature: str) -> pd.DataFrame:
        ...
        if feature in periods:
            ...
            df[f'{feature}_sin'] = self._sin_transformer(period).fit_transform(df[feature])
            df[f'{feature}_cos'] = self._cos_transformer(period).fit_transform(df[feature])
    ...
    def _add_special_day_features(self, df: pd.DataFrame, calendar_data: pd.DataFrame) -> pd.DataFrame:
    ...
    def get_feature_names(self) -> List[str]:
        ...
        features = (
            self.feature_config.time_features +
            [f"{feat}_{transform}" 
